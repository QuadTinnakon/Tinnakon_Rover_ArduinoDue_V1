...
//accrY_Earth = hy;
//accrZ_Earth = hz;
accrX_Earthf = (AccX*DCM00 + AccY*DCM01 + AccZ*DCM02)*-1.0;
accrY_Earthf = (AccX*DCM10 + AccY*DCM11 + AccZ*DCM12)*-1.0;
accrZ_Earthf = (AccX*DCM20 + AccY*DCM21 + AccZ*DCM22) - acc_offsetZ2;
accrX_Earth = accrX_Earth + (accrX_Earthf - accrX_Earth)*G_Dt*18.51245;
accrY_Earth = accrY_Earth + (accrY_Earthf - accrY_Earth)*G_Dt*18.51245;
accrZ_Earth = accrZ_Earth + (accrZ_Earthf - accrZ_Earth)*G_Dt*18.51245;//22.51245 42.5 18.5 Low pass filter ,smoothing factor  α := dt / (RC + dt)
